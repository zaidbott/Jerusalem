...
async def gsearch(q_event):
    ...
    zedevent = await edit_or_reply(q_event, "`searching........`")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await edit_delete(zedevent, f"**Error:**\n`{e}`", time=10)
    msg = ""
    for i in range(lim):
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        zedevent = await edit_or_reply(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await zedevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await zedevent.edit("`Unable to perform reverse search.`")
        ...
        if guess and imgspage:
            await zedevent.edit(f"[{guess}]({fetchUrl})\n\n`Looking for this Image...`")
        else:
            return await zedevent.edit("`Can't find any kind similar images.`")
